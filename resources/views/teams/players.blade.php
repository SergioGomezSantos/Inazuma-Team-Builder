    ...
    <div class="fixed left-1/2 transform -translate-x-1/2 top-30 z-50" x-data="{ showReorderNotification: localStorage.getItem('reorderNotificationDismissed') !== 'true' }"
        ...
        <div class="bg-gray-400 dark:bg-yellow-500 px-4 py-3 rounded-md shadow-lg flex items-center max-w-md">
            ...
            <button
                ...
                class="ml-4 text-white hover:text-gray-200 focus:outline-none" aria-label="Cerrar notificación">
                ...
                <div class="h-[calc(100vh-12rem)] flex flex-col rounded-lg bg-gray-100 dark:bg-gray-700">
                    ...
                    <div class="pt-11 pr-12 flex justify-end items-center">
                        <button id="toggleView"
                            ...
                            aria-label="Alternar vista entre estadísticas y técnicas">
                            <svg id="statsIcon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                class="w-5 h-5" stroke-width="1.5" stroke="currentColor">
                                    ...
                                    <tr class="dark:text-primary-500">
                                        ...
                                        @foreach (['ie1', 'ie2', 'ie3'] as $version)
                                            <th scope="col" class="px-0 py-3 text-center"
                                                data-col="stats-{{ $version }}">
                                                Stats {{ strtoupper($version) }}</th>
                                            <th scope="col" class="px-0 py-3 text-center"
                                                data-col="techniques-{{ $version }}">
                                                Técnicas {{ strtoupper($version) }}</th>
                                        @endforeach
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                                    @foreach ($players as $player)
                                        <tr>
                                            ...
                                            <td class="whitespace-nowrap ignore-sort" data-col="data">
                                                ...
                                                <div class="flex justify-center items-center gap-2">
                                                    <img src="{{ asset('storage/icons/positions/' . $player->position) . '.webp' }}"
                                                        class="w-10 h-10 object-contain" alt="{{ $player->position }}">
                                                    <img src="{{ asset('storage/icons/elements/' . $player->element) . '.webp' }}"
                                                        class="w-5 h-5 object-contain" alt="{{ $player->element }}">
                                                </div>
                                            ...
                                            @foreach (['ie1', 'ie2', 'ie3'] as $version)
                                                <!-- Stats Column -->
                                                <td class="whitespace-nowrap text-right p-12"
                                                    data-col="stats-{{ $version }}">
                                                    @php $stats = $player->stats->firstWhere('version', $version); @endphp
                                                    @if ($stats)
                                                        @foreach (array_keys($statLabels) as $key)
                                                            <div
                                                                class="flex justify-between items-center mb-2 gap-x-8 border-b-2 border-gray-200 dark:border-gray-500">
                                                                <span
                                                                    class="dark:text-yellow-500">{{ $stats->$key }}</span>
                                                    ...
                                                    <div class="flex flex-col items-center">
                                                        ...
                                                        <ul
                                                            ...
                                                            @foreach ($player->techniques as $technique)
                                                                @php
                                                                    $animeVersion =
                                                                        'anime' . str_replace('ie', '', $version);
                                                                @endphp
                                                                @if ($technique->pivot->source == $animeVersion)
                                                                    @php
                                                                        $withPlayers =
                                                                            json_decode(
                                                                    ...
                                                                    <li class="flex flex-col gap-1">
                                                                        ...
                                                                        @if (count($withPlayers) > 0)
                                                                            @php
                                                                                $lines = array_chunk($withPlayers, 3);
                                                                            @endphp

                                                                            <div
                                                                                class="flex flex-col gap-1 text-sm pl-1">
                                                                                @foreach ($lines as $line)
                                                                                    <div
                                                                                        class="flex gap-1 items-center ml-12">
                                                                                        <span>|</span>
                                                                                        @foreach ($line as $playerName)
                                                                                            @php
                                                                                                $playerExists =
                                                                                                    $players->firstWhere(
                                                                                                        'name',
                                                        ...
                                                        <ul class="space-y-4 border-l-2 pl-4 my-8 w-full max-w-xs">
                                                            @foreach ($player->techniques as $technique)
                                                                @if ($technique->pivot->source == $version)
